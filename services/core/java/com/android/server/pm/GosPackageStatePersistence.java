...
class GosPackageStatePersistence {
    ...
    private static final String ATTR_MICROPHONE_SCOPES = "microphone-scopes";
    ...
    private static void serializeInner(GosPackageState ps, TypedXmlSerializer serializer) throws IOException {
        ...
        if (ps.hasFlag(GosPackageStateFlag.MICROPHONE_SCOPES_ENABLED)) {
            byte[] s = ps.microphoneScopes;
            if (s != null) {
                serializer.attributeBytesHex(null, ATTR_MICROPHONE_SCOPES, s);
            }
        }
    ...
    static GosPackageState deserialize(TypedXmlPullParser parser) throws XmlPullParserException {
        ...
        for (int i = 0, numAttr = parser.getAttributeCount(); i < numAttr; ++i) {
            ...
            switch (attr) {
                ...
                case ATTR_MICROPHONE_SCOPES ->
                    microphoneScopes = parser.getAttributeBytesHex(i);
                default ->
                    Slog.e(TAG, "deserialize: unknown attribute " + attr);
            }
        }
        return new GosPackageState(flagStorage1, packageFlagStorage, storageScopes, contactScopes, microphoneScopes);
    }
    ...
    static GosPackageState maybeDeserializeLegacy(TypedXmlPullParser parser) {
        ...
        // microphoneScopes was not present in legacy format
