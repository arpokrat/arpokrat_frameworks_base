...
class GosPackageStatePermission {
    ...
    static final int FIELD_MIC_SPOOFING_CONFIG = 3;

    @IntDef(prefix = "FIELD_", value = {
            FIELD_STORAGE_SCOPES, FIELD_CONTACT_SCOPES, FIELD_PACKAGE_FLAGS, FIELD_MIC_SPOOFING_CONFIG
    })
    ...
    GosPackageState filterRead(GosPackageState ps) {
        ...
        var res = new GosPackageState(ps.flagStorage1 & readFlagStorage1
                ...
                , canReadField(FIELD_MIC_SPOOFING_CONFIG) ? ps.micSpoofingConfig : default_.micSpoofingConfig
        );
    ...
    GosPackageState filterWrite(GosPackageState current, GosPackageState update) {
        ...
        var res = new GosPackageState(
                ...
                , canWriteField(FIELD_MIC_SPOOFING_CONFIG) ? update.micSpoofingConfig : current.micSpoofingConfig
        );
        var default_ = GosPackageState.DEFAULT;
        if (default_.equals(res)) {
            return default_;
